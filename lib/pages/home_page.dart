  ...
  return GradientScaffold(
    ...
    body: RefreshIndicator(
      onRefresh: () async {
        ...
        setState(() {});                   // trigger rebuild, FutureBuilder sẽ gọi lại _fetchTransactionsSummary()
      },
      child: SingleChildScrollView(
        physics: const AlwaysScrollableScrollPhysics(), // quan trọng để có thể vuốt ngay cả khi ít nội dung
            ...
            TextSection(
              ...
              onAction: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => TransactionPage(), // trang danh sách giao dịch
                  ),
                );
              }
            ),
...
class _TransactionCardState extends State<TransactionCard> {
  ...
  Future<void> _fetchRecentTransactions() async {
    try {
      ...
      txs.sort((a, b) => DateTime.parse(b['transaction_date'])
  ...
  leading: CircleAvatar(
    ...
    child: Icon(
      categoryIcons[tx['category']['category_name']] ?? Icons.category,
  ...
  class MonthlyLineChart extends StatelessWidget {
    ...
    @override
    Widget build(BuildContext context) {
      final allValues = [...incomeSpots, ...expenseSpots].map((e) => e.y).toList();
      final double maxY =
          allValues.isNotEmpty ? (allValues.reduce(math.max) * 1.2) : 1000;
